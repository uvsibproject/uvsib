...
def get_options():
    ...
    job_script = settings.configs['codes']['SevenNet']['job_script']
    resources = {
...
SevenNetCalculation = CalculationFactory('sevennet')


class SevenNetWorkChain(BaseRestartWorkChain):
    """BaseRestartWorkChain to run SevenNetCalculation with automatic restarts."""

    _process_class = SevenNetCalculation
    ...
    def define(cls, spec):
        ...
        spec.expose_outputs(SevenNetCalculation)
        ...
        spec.exit_code(
            ...
            message='Maximum number of restarts exceeded for SevenNetWorkChain.'
        )
    ...
    def setup(self):
        ...
        self.ctx.inputs = {
            ...
            'metadata': {
                'options': get_options(),
                'label': 'SevenNet: {}'.format(self.inputs.local_label.value)
            }
        }
